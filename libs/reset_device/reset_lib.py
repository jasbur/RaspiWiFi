...
def reset_to_host_mode():
	...
	os.system('sudo cp -r /usr/lib/raspiwifi/reset_device/static_files/aphost_bootstrapper /etc/cron.raspiwifi/')
	os.system('chmod +x /etc/cron.raspiwifi/aphost_bootstrapper')
	os.system('sudo mv /etc/dhcpcd.conf /etc/dhcpcd.conf.original')
	os.system('sudo cp -r /usr/lib/raspiwifi/reset_device/static_files/dhcpcd.conf /etc/')
	os.system('sudo mv /etc/dnsmasq.conf /etc/dnsmasq.conf.original')
	os.system('sudo cp -r /usr/lib/raspiwifi/reset_device/static_files/dnsmasq.conf /etc/')
	os.system('sudo cp -r /usr/lib/raspiwifi/reset_device/static_files/dhcpcd.conf /etc/')
